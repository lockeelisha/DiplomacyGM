...
class OrderDrawer:
    ...
    def draw_order(
        ...
        if order_function is None:
            logger.debug(
                "None order found: %s drawn. Coordinates: %s",
                "hold" if current_turn.is_moves() else "disband",
                coordinate,
            )
            order_function = (
                self._draw_hold if current_turn.is_moves() else self.draw_force_disband
            )
        # Unit and order are optional and are _ or __ in the definitions of functions that don't need them
        return order_function(
            unit, order, coordinate, order.has_failed if order else False
        )
    ...
    def draw_player_order(self, order: PlayerOrder) -> None:
        ...
        coord_list = MapperUtils.get_all_unit_coordinates(
            order.province, unit_type, coast
        )
    ...
    def draw_retreat_move(
        self, unit: Unit, order: RetreatMove, coordinate: complex, _
    ) -> list[Element]:
        ...
        destination = MapperUtils.loc_to_point(
            order.destination,
            unit.unit_type,
        ...
        if order.destination.unit:
            destination = MapperUtils.pull_coordinate(
                coordinate, destination, 1.5 * self.board_svg_data["unit_radius"]
            )
    ...
    def _path_helper(
        ...
        for possibility in current.adjacencies.get_all():
            ...
            if is_convoying_fleet:
                options += self._path_helper(
                    source, destination, possibility, new_checked
                )
    ...
    def _draw_convoyed_move(
        self, unit: Unit, order: Move, coordinate: complex, has_failed: bool
    ) -> list[Element]:
        def f(point: complex):
            return f"{point.real},{point.imag}"

        valid_convoys = self.convoy_paths.get(
            unit.province, [[unit.province, order.destination]]
        )
        latest_paths = []
        for path in valid_convoys:
            ...
            if path[-1].unit:
                points[-1] = MapperUtils.pull_coordinate(
                    points[-2], points[-1], 1.5 * self.board_svg_data["unit_radius"]
                )
            ...
            for x in range(1, len(points) - 1):
                ...
                control_point = vectors[0] / abs(vectors[0]) - vectors[1] / abs(
                    vectors[1]
                )
                control_point = (
                    points[x]
                    if control_point == 0
                    else points[x] + control_point / abs(control_point) * 30
                )
            ...
            latest_paths.append(
                self._draw_path(
                    stroke, marker_end=marker_color, stroke_color=stroke_color
                )
            )
        return latest_paths

    def _draw_support(
        self, unit: Unit, order: Support, coordinate: complex, has_failed: bool
    ) -> list[Element]:
        ...
        source_coord = MapperUtils.loc_to_point(
            source,
            unit.unit_type,
            source_coast,
            coordinate,
        ...
        dest_coord = MapperUtils.loc_to_point(
            order.destination,
            source_unit_type,
            dest_coast,
            source_coord,
        ...
        if order.destination.unit:
            dest_coord = MapperUtils.pull_coordinate(
                coordinate if order.is_support_hold() else source_coord,
                dest_coord,
                (1 if order.is_support_hold() else 1.5)
                * self.board_svg_data["unit_radius"],
            )
            # Draw hold around unit that can be support-held
            if (
                order.is_support_hold()
                and source.unit is not None
                and isinstance(source.unit.order, (ConvoyTransport, Support))
                and MapperUtils.is_moveable(
                    source.unit, self.adjacent_provinces, self.player_restriction
                )
            ):
                for coord in MapperUtils.get_all_unit_coordinates(
                    source, source.unit.unit_type, source.unit.coast
                ):
            ...
            if (
                isinstance(destunit.order, Support)
    ...
    def draw_force_disband(self, _, __, coordinate: complex, ___, svg=None) -> None:
        ...
        points = np.concatenate(
            (init, init @ rotate_90, -init, -init @ rotate_90)

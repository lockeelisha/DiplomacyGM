...
class AdjacencyData:
	...
	def __init__(self):
		...
		self.adjacent_by_type: dict[str | Terrain, set[Province]] = {
			u: set() for u in Terrain
		}
	...
	def add_coast(
		self, province: Province, origin_coast: str | None, dest_coast: str | None
	):
	...
	def remove(self, province: Province, terrain: Terrain | None = None):
		...
		origin_coasts = {
			origin
			for origin, _ in self.adjacencies[province].coasts
	...
	def get_coasts(
		self, province: Province, coast: str | None = None
	) -> set[str | None]:
		...
		if coast is not None:
			return {
				dest_coast
				for origin_coast, dest_coast in adj.coasts
				if origin_coast == coast
			}
		return {dest_coast for _, dest_coast in adj.coasts if dest_coast is not None}

	def get_all(
		self, terrain: Terrain | set[Terrain] | None = None, coast: str | None = None
	) -> set[Province]:
		...
		return {
			province
			for key in keys
			for province in self.adjacent_by_type.get(key, set())
		}

	def get_all_with_coasts(
		self, coast: str | None
	) -> set[tuple[Province, str | None]]:

...
class PanelDrawer:
	...
	def __init__(
		...
		for power_element in all_power_banners_element:
			destination_pretransform_coordinates = TransGL3(power_element[0]).transform(
				get_coordinates(power_element[0])
			)
			destination_coordinates = TransGL3(power_element).transform(
				destination_pretransform_coordinates
			)
	...
	def _draw_power_banner(
		self,
		power_element: Element,
		player: Player,
		banner_index: int,
		high_player_count: bool,
		display_scs: bool,
	) -> bool:
		if len(power_element) == 0:
			return False
		initial_pretransform_coordinates = TransGL3(power_element[0]).transform(
			get_coordinates(power_element[0])
		)
		banner_coordinates = TransGL3(power_element).transform(
			initial_pretransform_coordinates
		)
		if (
			high_player_count
			and banner_coordinates != self.scoreboard_power_locations[banner_index]
		):
			return False
		if (
			not high_player_count
			and get_element_color(power_element[0]) != player.default_color
		):
			return False
		...
		sort_powers &= (
			self.board.data.get("fow", "disabled") != "enabled"
			or self.restriction is None
		)
		if sort_powers:
			original_trans = TransGL3(power_element)
			translation = self.scoreboard_power_locations[
				banner_index
		...
		for index, value in (
			self.board_svg_data["scoreboard"].get("indexes", {}).items()
		):
			if not index.isnumeric() or (i := int(index)) >= len(power_element):
				continue
			if (
				value == "__name__"
				and not high_player_count
				and not player_data.get("nickname")
			):
				continue
			...
			value = value.replace(
				"__pct__", "{:.0f}%".format(self.board.get_score(player) * 100)
			)
	...
	def _draw_side_panel_scoreboard(self, svg: ElementTree) -> None:
		...
		all_power_banners_element = find_svg_element(
			root, "power_banners", self.board_svg_data
		)
		if all_power_banners_element is None:
			return

		high_player_count = (
			len(self.board.get_players()) > len(self.scoreboard_power_locations)
		...
		for i, player in enumerate(self.board.get_players_sorted_by_score()):
			if i >= len(self.scoreboard_power_locations):
				break
			for power_element in all_power_banners_element:
				display_scs = self.board.data.get(
					"fow", "disabled"
				) != "enabled" or self.restriction in {player, None}
				if self._draw_power_banner(
					power_element, player, i, high_player_count, display_scs
				):
					break

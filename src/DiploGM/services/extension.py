...
class ExtensionService(BaseService):
    ...
    def record_extension(
        ...
        event = ExtensionEvent(
            user_id=user_id, server_id=game_id, hours=hours, reason=reason
        )
    ...
    def view_user_extensions(self, user_id: int) -> dict:
        ...
        servers = {}
        for e in sorted(
            events, key=lambda e: (e.server_id, e.created_at), reverse=True
        ):

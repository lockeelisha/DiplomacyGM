...
async def grace_log(
	ctx: commands.Context, user: discord.User, hours: float, reason: str = "Unspecified"
) -> None:
...
async def grace_view_user(ctx: commands.Context, user: discord.User) -> None:
	...
	if len(extensions) == 0:
		...
		await send_message_and_file(
			channel=ctx.channel, title=f"Graces caused by {user.name}", message=out
		)
		return
...
async def grace_view_server(
	ctx: commands.Context, server_id: Optional[int] = None
) -> None:
	...
	if len(extensions) == 0:
		...
		await send_message_and_file(
			channel=ctx.channel, title=f"Graces caused by {gname}", message=out
		)
		return

	sorted_events = sorted(
		extensions.items(), key=lambda p: max(e.created_at for e in p[1]), reverse=True
	)

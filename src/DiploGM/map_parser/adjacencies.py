...
def _get_adjacent_geom(province: Province) -> set[Province]:
	return {
		a
		for a in province.adjacencies.get_all()
...
def _find_province_loop(
	province: Province,
	destination: Province,
	visited: list[Province],
	ignored_provinces: set[Province],
) -> Optional[list[Province]]:
...
def _check_missing_adjacencies(
	province: Province, adj: Province, visited_provinces: set
) -> str | None:
	...
	if len(common_adj) == 1:
		...
		return (
			f"Found a loop of provinces {', '.join(p.name for p in loop)}. "
...
def _check_cliques(
	...
	warnings = []
	for third, fourth in combinations(
		_get_adjacent_geom(province) & _get_adjacent_geom(adj) - visited_provinces, 2
	):
		...
		if not all_adjacent:
			...
			warnings.append(
				f"Provinces {province.name}, {adj.name}, {third.name}, "
...
def verify_adjacencies(variant: str) -> str:
	...
	for province_name, province_data in (
		board.data.get("overrides", {}).get("provinces", {}).items()
	):
	...
	for province in [p for p in board.provinces if p.name[-1] in "23456789"]:
		try:
			...
			if comp_province.adjacencies.get_all() ^ province.adjacencies.get_all() != {
				...
				warnings.append(
					f"Province {province.name} and {comp_province.name} have different adjacencies"
				)
			visited_provinces.add(province)
		except ValueError:
			warnings.append(
				f"Province {province.name} is named like a high seas province "
	...
	for province in board.provinces - high_provinces:
		...
		for adj in _get_adjacent_geom(province) - visited_provinces:
			warnings.extend(
				_check_cliques(province, adj, visited_provinces | visited_adjacent)
			)

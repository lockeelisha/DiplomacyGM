...
class Parser:
	...
	def _load_config(self, variant_name: str) -> dict:
		...
		with open(
			f"{parse_variant_path(variant_name)}/config.json", "r", encoding="utf-8"
		) as f:
			variant_data = json.load(f)
		try:
			with open(
				f"{parse_variant_path(variant_name, return_parent=True)}/config.json",
				"r",
				encoding="utf-8",
			) as f:
				data = json.load(f)
	...
	def _load_unit_types(self) -> dict[str, UnitType]:
		...
		for folder in [
			parse_variant_path(self.datafile, return_parent=True),
			parse_variant_path(self.datafile),
		]:
			if os.path.isdir(f"{folder}/units"):
				unit_files.extend(
					[
						f"{folder}/units/{f}"
						for f in os.listdir(f"{folder}/units")
						if f.endswith(".toml")
					]
				)
			...
			with open(unit_file, "rb") as f:
				...
				unit_types[unit_code] = UnitType(
					...
					moves_on={
						...
						for terrain in unit_data.get("moves_on", ["Land"])
					},
					transforms_to=None,
				)
	...
	def _create_retreat_layer(
		self, svg_root: etree._ElementTree, layer_name: str, config_data: dict
	) -> Element:
		...
		print(
			f"Retreat layer {layer_name} not found. Creating one by copying {move_layer_name} layer."
		)
		move_layer = find_svg_element(svg_root, move_layer_name, config_data)
		if move_layer is None:
			raise ValueError(
				f"Neither {layer_name} nor {move_layer_name} layers were found in the SVG"
			)
		retreat_layer = copy.deepcopy(move_layer)
		retreat_layer.set(
			"id", config_data.get(layer_name, f"{move_layer_name}_retreat")
		)
		retreat_layer.set(
			f"{NAMESPACE.get('inkscape')}label",
			f"{move_layer_name.capitalize()} Locations (Retreats)",
		)
		translation = -self.data[SVG_CONFIG_KEY].get("unit_radius", 0)
		retreat_layer.set(
			"transform",
			f"translate({translation}, {translation}) {retreat_layer.get('transform', '')}",
		)
		...
		for layer in LAYER_NAMES:
			...
			if l is None:
				if layer in {"retreat_army", "retreat_fleet"}:
					logger.info(
						"Layer %s not found in SVG. Duplicating army/fleet layer.",
						layer,
					)
	...
	def verify_svg(self) -> list[str]:
		...
		for province, data in (
			self.data.get("overrides", {}).get("high provinces", {}).items()
		):
		...
		for layer_name in [
			...
			for element in layer:
				...
				name = re.sub(
					r" \(?[ensw]+c\)?$", "", name, flags=re.IGNORECASE
				)  # Remove coast names
				if name not in seen_names:
					error = (
						f"[{layer_name}] Name '{name}' not found in any province layer"
					)
					logger.error(error)
					errors.append(error)
	...
	def parse(self, **kwargs) -> Board:
		...
		initial_turn = Turn(
			self.data["year"], PhaseName.SPRING_MOVES, self.data["year"]
		)
		...
		if "victory_count" not in self.data:
			self.data["victory_count"] = int(
				(len([1 for p in provinces if p.has_supply_center]) + 1) / 2
			)
		...
		for player in self.players:
			...
			if "iscc" not in game_data["players"][player.name]:
				game_data["players"][player.name]["iscc"] = len(
					[1 for p in provinces if p.has_supply_center and p.owner == player]
				)
	...
	def _read_map(self) -> tuple[set[Province], set[tuple[str, str]]]:
		...
		for province in provinces:
			...
			if (
				self.data.get("convoyable_islands") == "enabled"
				and province.type == ProvinceType.ISLAND
			):
	...
	def _json_cheats(self, provinces: set[Province]) -> set[Province]:
		...
		offset = complex(
			self.data[SVG_CONFIG_KEY].get("loc_x_offset", 0),
		...
		for name, data in self.data["overrides"].get("provinces", {}).items():
			...
			if not province:
				logger.debug(
					"Province %s in overrides not found in map, skipping...", name
				)
				continue
			...
			for index, coordinate in enumerate(unit_locs):
				...
				retreat_coord = (
					retreat_locs[index] if index < len(retreat_locs) else coordinate
				)
	...
	def _get_provinces(
		self, provinces: set[Province], adjacencies: set[tuple[str, str]]
	) -> set[Province]:
		...
		for province in provinces:
			...
			province.unit_coordinates["default"] = UnitLocation(
				primary_coordinate=center, retreat_coordinate=center
			)
			for unit in province.unit_coordinates.keys():
				province.all_coordinates.setdefault(unit, set()).add(
					province.unit_coordinates[unit]
				)
	...
	def _get_province_coordinates(self) -> set[Province]:
		...
		for layer_name, province_type in province_types.items():
			...
			for province_data in list(cur_layer):
				provinces.add(
					self._create_province(
						province_data, province_type, layer_transformation
					)
				)
		return provinces
	...
	def _create_province(
		...
		if not path_string:
			logger.error(
				"Province path data not found in province with data %s",
				tostring(province_data),
			)
		...
		if len(province_coordinates) <= 1:
			...
			poly = shapely.MultiPolygon(
				list(
					map(
						lambda coords: shapely.Polygon(
							[(p.real, p.imag) for p in coords]
						),
		...
		if self.layers["province_labels"]:
			...
			if name == "":
				raise RuntimeError(
					f"Province name not found in province with data {tostring(province_data)}"
				)
	...
	def _initialize_province_owners(self) -> None:
		...
		for province_data in layers:
			...
			if self.name_to_province[name].is_impassable:
				continue
			self.name_to_province[name].owner = self._get_element_player(
				province_data, province_name=name
			)
	...
	def _initialize_province_names(self, provinces: set[Province]) -> None:
		def set_province_name(
			province: Province, name_data: etree.Element, _: str | None
		) -> None:
			if province.name != "":
	...
	def _initialize_supply_centers(self) -> None:
		...
		for center_data in self.layer_data["supply_center_icons"]:
			...
			if not shapely.dwithin(
				sc_point,
				province.geometry,
				self.data[SVG_CONFIG_KEY].get("unit_radius", 10),
			):
				logger.warning(
					"%s: Supply center icon for '%s' is not within its province",
					self.datafile,
					name,
				)
			...
			if not self.is_chaos:
				...
				sc_circles.extend(
					center_data.findall(".//svg:path", namespaces=NAMESPACE)
				)
				if len(sc_circles) > 0:
					element = next(
						(
							c
							for c in sc_circles
							if get_element_color(c) not in (None, "none", "000000")
						),
						sc_circles[-1],
					)
				else:
	...
	def _set_province_unit(
		self, province: Province, unit_data: Element, coast: str | None = None
	) -> None:
		if province.unit:
			return
	...
	def _set_phantom_unit_coordinates(self) -> None:
		...
		for layer_name, unit_type, is_retreat in layers:
			...
			for unit_data in list(layer):
				...
				province, coast = self._get_province_and_coast(
					self.get_province_name(unit_data)
				)
				coordinate = get_element_unit_coordinates(unit_data)
				translated_coordinate = layer_translation.transform(
					unit_translation.transform(coordinate)
				)
				province.set_unit_coordinate(
					translated_coordinate, unit_type, is_retreat, coast
				)
	...
	def _get_province_and_coast(
		self, province_name: str
	) -> tuple[Province, str | None]:
		coast_suffix: str | None = None

		pattern = re.compile(
			r"^(.*?)\s*(?: \(([neswc]+c)\)| ([neswc]+c))$", re.IGNORECASE
		)
		...
		try:
			...
			for province1, province2 in itertools.combinations(provinces, 2):
				if shapely.dwithin(
					province1.geometry,
					province2.geometry,
					self.layers["border_margin_hint"],
				):
	...
	def _get_element_player(
		self, element: Element, province_name: str = ""
	) -> Player | None:
		...
		if self.is_chaos:
			if (
				color in (None, self.impassable_color)
				or not self.name_to_province[province_name].has_supply_center
			):
				return None
	...
	def _get_unit_type(self, unit_data: Element) -> str:
		...
		if self.data[SVG_CONFIG_KEY]["unit_type_labeled"]:
			...
			raise RuntimeError(
				f"Unit types are labeled, but {name} doesn't start with F or A"
			)

		if (
			"unit_type_from_names" in self.data[SVG_CONFIG_KEY]
			and self.data[SVG_CONFIG_KEY]["unit_type_from_names"]
		):
			name = unit_data[1].get(INKSCAPE_LABEL)
			if name is None:
				raise RuntimeError(
					"Unit has no label, but unit_type_from_names is enabled"
				)
			if name.lower() in data_to_unit:
				return data_to_unit[name.lower()]
			raise RuntimeError(
				f"Unit types are labeled, but {name} wasn't sail or shield"
			)

		unit_data = unit_data.findall(".//svg:path", namespaces=NAMESPACE)[-1]
		num_sides = unit_data.get(

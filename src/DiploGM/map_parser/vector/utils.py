...
LAYER_DICTIONARY = {
	...
	"other_fills": {
		"Other Fills",
		"OTHER FILLS (High Seas)",
		"OTHER FILLS (Impassables and High Seas)",
	},
	"season": {"Season Title"},
	"year": {"Year Title"},
...
def find_svg_element(
	svg_root: Element | ElementTree, layer_name: str, config_data: dict
) -> Element | None:
	...
	for element_label in LAYER_DICTIONARY.get(layer_name, set()) | {layer_name}:
		if (
			element := svg_root.find(
				f'*[@inkscape:label="{element_label}"]',
...
def _parse_path_command(
	...
	if command in ["m", "c", "l", "t", "s", "q", "a"]:
		return (
			args if is_absolute else coordinate + args
		)  # Ignore all args except the last
	if command == "h":
...
def parse_path(path_string: str, translation: TransGL3) -> list[list[complex]]:
	...
	while current_index < len(path):
		if path[current_index][0].isalpha():
			...
			if (
				expected_arguments := arguments_by_command.get(command.lower())
			) is None:
	...
	for province in provinces:
		...
		for resident_data in resident_dataset:
			...
			if not point:
				...
				continue
			if province.geometry is not None and province.geometry.contains(

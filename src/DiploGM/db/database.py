...
class _DatabaseConnection:
	...
	def load_board(self, board_id: int) -> Board | None:
		...
		for board_row in board_data:
			...
			if current_turn is None:
				logger.warning(
					"Could not parse turn string '%s' for board %s",
					phase_string,
					board_id,
				)
				continue
			...
			board.turn = Turn(
				board.data["year"] + board.turn.year,
				board.turn.phase,
				board.data["year"],
			)
	...
	def get_board(
		...
		board = self._get_board(
			board_id, turn, data_file, cursor, clear_status=clear_status
		)
		cursor.close()
		return board
	...
	def _load_province(
		self, board: Board, province: Province, province_info_by_name: dict
	):
		...
		province.core_data.half_core = (
			board.get_player(half_core) if half_core is not None else None
		)
		province.unit = None
	...
	def _load_dp_orders(self, board: Board, dp_data: tuple):
		...
		if player is None:
			logger.warning(
				"Couldn't find player %s for DP order at %s", player_name, location
			)
			return
		try:
	...
	def _get_board(
		...
		board = parser_result.parse(
			chaos=chaos_query[0] == "enabled" if chaos_query else False
		)
		board.turn = turn
		...
		if clear_status:
			cursor.execute(
				"UPDATE units SET failed_order=False WHERE board_id=? and phase=?",
				(board_id, format(board.turn, "%I %S")),
			)
			cursor.execute(
				"UPDATE builds SET failed_order=False WHERE board_id=? and phase=?",
				(board_id, format(board.turn, "%I %S")),
			)
		...
		cursor.executemany(
			...
			[
				(
					board_id,
					player.name,
					board.data["players"][player.name].get(
		...
		cursor.executemany(
			...
			[
				(
					...
					province.core_data.half_core.name
					if province.core_data.half_core
					else None,
				)
				for province in board.provinces
			],
		...
		cursor.executemany(
			...
			[
				(
					...
					(
						(
							build_order.unit_type.code
							if isinstance(build_order, Build)
							else ""
						)
		...
		cursor.executemany(
			...
			[
				(
					...
					dp_order.order.get_destination_str()
					if dp_order.order is not None
					else None,
					dp_order.order.get_source_str()
					if dp_order.order is not None
					else None,
				)
				for unit in board.units
	...
	def save_board_state(self, board_id: int, board: Board):
		...
		cursor.execute(
			"DELETE FROM units WHERE board_id=? AND phase=?", (board_id, phase)
		)
		...
		cursor.executemany(
			...
			[
				(
					...
					format(board.turn, "%I %S"),
					unit.province.get_name(unit.coast),
					f"{unit.province.get_name()} coast"
					if not unit.coast
					else None,  # Legacy coast support
	...
	def load_ctx_parameters(self) -> dict:
		...
		result = cursor.execute(
			"SELECT context_id, parameter_key, parameter_value FROM ctx_parameters"
		).fetchall()
	...
	def delete_board(self, board: Board):
		...
		phase = format(board.turn, "%I %S")
		cursor.execute(
			"DELETE FROM boards WHERE board_id=? AND phase=?", (board.board_id, phase)
		)
		cursor.execute(
			"DELETE FROM provinces WHERE board_id=? AND phase=?",
			(board.board_id, phase),
		)
		cursor.execute(
			"DELETE FROM units WHERE board_id=? AND phase=?", (board.board_id, phase)
		)
		cursor.execute(
		...
		cursor.execute(
			"DELETE FROM builds WHERE board_id=? AND phase=?", (board.board_id, phase)
		)
		cursor.execute(
			"DELETE FROM retreat_options WHERE board_id=? AND phase=?",
			(board.board_id, phase),

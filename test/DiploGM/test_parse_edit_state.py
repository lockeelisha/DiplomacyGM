"""Test module for the Parse Edit State functionality."""
import unittest
...
class TestParseEditState(unittest.TestCase):
    def test_set_core(self):
        ...
        self.assertEqual(b.board.get_province("Belgium").core_data.core,
                         b.players["England"], "Failed to set core ownership for Belgium")
        _parse_command("set_core Belgium None", b.board)
        self.assertIsNone(b.board.get_province("Belgium").core_data.core,
                          "Failed to remove core ownership for Belgium")

    def test_set_half_core(self):
        """Tests the set_half_core command."""
        b = BoardBuilder()
        _parse_command("set_half_core Spain France", b.board)
        self.assertEqual(b.board.get_province("Spain").core_data.half_core,
                         b.players["France"], "Failed to add half-core ownership for Spain")
    ...
    def test_set_province_owner(self):
        ...
        self.assertEqual(b.board.get_province("Warsaw").owner,
                         b.players["Austria"], "Failed to set owner of Warsaw to Austria")
    ...
    def test_set_total_owner(self):
        """Tests the set_total_owner command, which sets both the owner and core of a province."""
        b = BoardBuilder()
        _parse_command("set_total_owner Vienna Germany", b.board)
        self.assertEqual(b.board.get_province("Vienna").owner,
                         b.players["Germany"], "Failed to set owner of Vienna to Germany")
        self.assertEqual(b.board.get_province("Vienna").core_data.core,
                         b.players["Germany"], "Failed to set core of Vienna to Germany")
    ...
    def test_create_dislodged_unit(self):
        """Tests the create_dislodged_unit command."""
        b = BoardBuilder()
        ...
        self.assertIn((p_trieste, None), a_serbia.retreat_options,
                      "Dislodged unit in Serbia missing retreat option to Trieste")
        self.assertIn((p_budapest, None), a_serbia.retreat_options,
                      "Dislodged unit in Serbia missing retreat option to Budapest")
        ...
        self.assertFalse(f_bulgaria.retreat_options,
                         "Dislodged Fleet unit in Bulgaria_ec should have no retreat options")

    def test_delete_unit(self):
        """Tests the delete_unit command."""
        b = BoardBuilder()
        f_liverpool = b.fleet("Liverpool", b.players["England"])
    ...
    def test_move_unit(self):
        """Tests the move_unit command."""
        b = BoardBuilder()
        a_berlin = b.army("Berlin", b.players["Germany"])
        ...
        self.assertEqual(b.board.get_province("Prussia").unit, a_berlin,
                         "New province Prussia does not have the moved unit")

    def test_dislodge_unit(self):
        """Tests the dislodge_unit command."""
        b = BoardBuilder()
        b.board.turn = Turn(1901, PhaseName.SPRING_RETREATS)
        b.army("Munich", b.players["Germany"])
        ...
        self.assertEqual(p_munich.dislodged_unit.player, b.players["Germany"],
                         "Dislodged unit in Munich does not belong to Germany")

    def test_make_units_claim_provinces(self):
        """Tests the make_units_claim_provinces command,
        which sets the owner of a province to the player of the unit in that province."""
        b = BoardBuilder()
        b.fleet("Tunis", b.players["Italy"])
        b.army("North Africa", b.players["Italy"])
        ...
        self.assertNotEqual(p_tunis.owner, b.players["Italy"],
                            "Tunis should not be owned by Italy, as it is a supply center")
        self.assertEqual(p_north_africa.owner, b.players["Italy"], "North Africa should be owned by Italy")

        _parse_command("make_units_claim_provinces true", b.board)
        self.assertEqual(p_tunis.owner, b.players["Italy"],
                         "Tunis should be owned by Italy after claiming all supply centers")
